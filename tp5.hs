...
splitMap cs = (h ++ [(country, color)] ++ t, h ++ [(country, colors)] ++ t)
  where
    h                         = takeWhile (not . isSplitableColors . snd) cs
    (country,countryColors):t = dropWhile (not . isSplitableColors . snd) cs
    (color,colors)            = splitColors countryColors
...
filterBorders cs bs = [b | b <- bs, fst b `elem` cs, snd b `elem` cs]
...
writeCountry country colors (c:cs) | fst c == country = (fst c, colors) : writeCountry country colors cs
...
searchMap bs m | isSplitableMap m = let (m1,m2) = splitMap m in filter (not . isEmptyMap) $ searchMap bs (globalArcConsistency bs m1) ++ searchMap bs (globalArcConsistency bs m2)
               | otherwise        = [m]
